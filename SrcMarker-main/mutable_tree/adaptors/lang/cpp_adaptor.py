...
def convert_expression(node: tree_sitter.Node) -> Expression:
    # [防守] 处理未知或不支持的节点类型（像 JavaScript Adaptor 一样）
    if node.type == "comment":
        # 注释不是表达式，返回空字面量
        return node_factory.create_literal("")
    ...
    # [防守] 如果节点类型不在字典中，打印警告并返回默认值
    if node.type not in expr_convertors:
        import sys
        print(f"[WARNING] C++ convert_expression 遇到未知节点类型: {node.type}，返回空字面量", file=sys.stderr)
        return node_factory.create_literal("")
...
def convert_statement(node: tree_sitter.Node) -> Statement:
    # [防守] 处理注释节点（像 JavaScript Adaptor 一样）
    if node.type == "comment":
        return convert_empty_stmt(node)
    ...
    # [防守] 如果节点类型不在字典中，打印警告并返回空语句
    if node.type not in stmt_convertors:
        import sys
        print(f"[WARNING] C++ convert_statement 遇到未知节点类型: {node.type}，返回空语句", file=sys.stderr)
        return convert_empty_stmt(node)
...
def convert_if_stmt(node: tree_sitter.Node) -> IfStatement:
    ...
    if alternative_node is None:
        ...
        # C++中alternative可能是'else_clause'节点，需要特殊处理
        # 类似JavaScript的处理方式：else_clause -> 'else', statement
        if alternative_node.type == "else_clause":
            # else_clause包含'else'关键字和实际的statement
