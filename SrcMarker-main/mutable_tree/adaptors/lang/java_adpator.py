...
def convert_if_stmt(node: tree_sitter.Node) -> IfStatement:
    ...
    # Java中condition实际上是parenthesized_expression，包含 ( expression )
    # 所以我们需要提取中间的表达式
    if cond_node.type == "parenthesized_expression":
        # parenthesized_expression: ( expression )
        assert cond_node.child_count == 3, f"Expected 3 children for parenthesized_expression, got {cond_node.child_count}"
        cond_node = cond_node.children[1]
    else:
        # 备选情况（虽然不应该发生）
        assert cond_node.type == "condition" and cond_node.child_count == 3, f"Unexpected condition type: {cond_node.type}"
        cond_node = cond_node.children[1]
