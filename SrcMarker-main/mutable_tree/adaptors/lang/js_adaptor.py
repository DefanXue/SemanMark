...
def convert_expression(node: tree_sitter.Node) -> Expression:
    # 跳过注释节点（修复KeyError: 'comment' in expressions）
    if node.type == "comment":
        # 注释不是表达式，返回空字面量
    ...
    expr_convertors = {
        ...
        "function_expression": convert_function_expression,  # 使用专门的转换函数
...
def convert_function_expression(node: tree_sitter.Node) -> FunctionDeclaration:
    """
    转换function_expression（匿名函数表达式）
    与convert_function_declaration使用相同的逻辑
    """
    header = convert_function_header(node)
    body = convert_statement(node.child_by_field_name("body"))
