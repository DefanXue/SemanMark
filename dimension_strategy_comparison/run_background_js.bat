...
if not exist " dimension_strategy_comparison\\logs\ mkdir \dimension_strategy_comparison\\logs\
if not exist \dimension_strategy_comparison\\logs\\js\ mkdir \dimension_strategy_comparison\\logs\\js\

:: Set environment variables
set CONFIG=dimension_strategy_comparison/configs/base_config_js.json
set PYTHON=python
set LOGDIR=dimension_strategy_comparison\\logs\\js
...
%PYTHON% dimension_strategy_comparison/test_strategy6_pipeline.py --config %CONFIG% --concurrency 10 --resume > %LOGDIR%\\log6_js_pipeline.txt 2>&1
...
%PYTHON% dimension_strategy_comparison/scripts/step4c_extract_with_llm_attacks.py --config %CONFIG% --concurrency 1 --resume > %LOGDIR%\\log6_js_llm.txt 2>&1
...
echo.
echo Results saved to: dimension_strategy_comparison\\results\\strategy_6_adaptive_js\\
echo.
